...
st.sidebar.header('Enter your Desirable Parameters:')

inputCamera  = st.sidebar.slider("Please Enter Camera in pixels: ", 10, 200, 64)
inputRAM     = st.sidebar.slider("Please Enter Memory in GB: ", 2,2048, 256)
inputScreen  = st.sidebar.slider("Please Enter Screen in inches: ", 3.0, 10.0, 6.7)
inputBattery = st.sidebar.slider("Please Enter Battery in mAh: ", 1000, 10000, 7000)
...
def finalOperation(brandName):
    ...
    st.write ("*******************************************************************")
    color = 'orange'
    st.write ("Prediction Price for your <<%s>> Phone is <<%s>> Tomans !!! " % (brandName.upper().center(8), answer.center(13)) )
